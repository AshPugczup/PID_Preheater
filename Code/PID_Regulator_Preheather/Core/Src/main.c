    ...
    /* USER CODE BEGIN 3 */
		...
		if(temp_set){
			...
			previous_error = PID_error; // Remember to store the previous error.
										//			if(zero_cross_detected && temp_set){ //TODO pomyśleć nad przełożeniem tego do interrupt
										//				time_stamp = Time;
										//				while((uint32_t)(Time - time_stamp) <= (maximum_firing_delay - PID_value));
